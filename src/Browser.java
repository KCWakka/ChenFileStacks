...
public class Browser {
    ...
    public void goToWebpage(String name) {
        ...
        if (has) {
            currentPage = internet.get(index);
        } else {
            Webpage error = new Webpage(name, "404 Error:\nPage not found");
            currentPage = error;
        }
        displayCurrentPage();
    }
    ...
    public void viewBrowserBackAndForward() {
        if (browserBack.isEmpty() && browserForward.isEmpty()) {
            System.out.println("There is no browsing history to view!");
        } else {
            System.out.print("Browser Back: \n" + browserBack.toString());
            System.out.println("Browser Foward: \n" + browserForward.toString());
        }
    }
